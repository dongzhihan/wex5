 ...
 navigator.app.exitApp();
  ...
  justep.Shell.closePage();
	...
	Model.prototype.modelParamsReceive = function(event){
      	...
      	Model.prototype.QUERY_TOOL = function(event){
      		justep.Shell.showPage("selectGoods");
      	};

       

      	Model.prototype.MOVE_TOOL = function(event){
      		justep.Shell.showPage("transferMain");
      	};

       

      	Model.prototype.INPUT_VALIDITY_TOOL = function(event){
	...
	Model.prototype.panel1Keydown = function(event){
        switch(event.keyCode)
        {
          case 49:
           ...
           if($("#buttonQUERY_TOOL").length>0)
           {	
                $("#buttonQUERY_TOOL").click();
           }
           break;
           case 50:
              if($("#buttonMOVE_TOOL").length>0)
           {	
                $("#buttonMOVE_TOOL").click();
           }
            break;
               case 51:
...
$(this.getElementByXid("panel1")).focus();
	...
	Model.prototype.modelLoad = function(event){
          var data =eval("("+sessionStorage["INTERNAL_TOOL"]+")");
